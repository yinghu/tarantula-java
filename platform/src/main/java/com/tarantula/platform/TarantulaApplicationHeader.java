...
public class TarantulaApplicationHeader implements TarantulaApplication,InstanceRegistry.Listener {
    ...
    public void onError(Session session, Exception ex) {
        ...
        session.write(this.builder.create().toJson(new ResponseHeader("onError",false,400,ex.getMessage(),"error")).getBytes(),this.descriptor.responseLabel());
    }
    ...
    public void refund(String systemId,String applicationId){
        ...
        this.context.publish(this.context.routingKey(systemId,"presence"),onBalanceTrack);
    }
