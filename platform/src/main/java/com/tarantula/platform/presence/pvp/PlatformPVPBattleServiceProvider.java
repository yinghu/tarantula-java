...
public class PlatformPVPBattleServiceProvider extends PlatformItemServiceProvider implements Configurable.Listener<SeasonCredentialConfiguration>{
    ...
    public void onLoaded(SeasonCredentialConfiguration loaded){
        ...
        seasonStartTime = loaded.startTime().toLocalTime();
        ...
        try{
            ...
            if(scheduleStore.mapRemove(lockKey)==null){
                ...
                if(next==null){
                    ...
                    LocalDateTime closeTime = LocalDate.now().atTime(seasonStartTime).plusSeconds(runtimeConfiguration.seasonRunningTime.get());
                    seasonRuntime.closeTime = TimeUtil.toUTCMilliseconds(closeTime);
                    seasonRuntime.endTime = TimeUtil.toUTCMilliseconds(closeTime.plusSeconds(seasonTimeGap));
    ...
    private void initialSeason(SeasonRuntime seasonRuntime){
        ...
        //LocalDate.now().atTime(seasonRuntime.)
        LocalDateTime closeTime = LocalDate.now().atTime(seasonStartTime).plusSeconds(runtimeConfiguration.seasonRunningTime.get());
        seasonRuntime.closeTime = TimeUtil.toUTCMilliseconds(closeTime);
        seasonRuntime.endTime = TimeUtil.toUTCMilliseconds(closeTime.plusSeconds(seasonTimeGap));
        dataStore.update(seasonRuntime);
