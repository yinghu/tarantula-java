...
public class StatisticsTask implements JDBCTask<Statistics>{
    ...
    public boolean execute(Statistics content) {
        try(Connection connection = pool.connection();
            PreparedStatement cmd = connection.prepareStatement("INSERT INTO "+content.label().replaceAll("-","_")+" (category,total,node,updated) VALUES(?,?,?,?) ON CONFLICT(category,node) DO UPDATE SET total = ?, updated = ?")) {
